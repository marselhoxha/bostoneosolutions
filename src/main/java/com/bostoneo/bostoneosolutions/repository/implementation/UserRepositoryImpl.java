   ...
   private String getVerificationUrl(String key, String type){
    ...
    public User verifyAccountKey(String key) {
        try {
            ...
            //Delete after updating, depends on the business logic
            return user;
    ...
    public User toggleMfa(String email) {
        User user = getUserByEmail(email);
        if(isBlank(user.getPhone())) {  throw new ApiException("Phone number is required to change MFA"); }
    ...
    private void saveImage(String email, MultipartFile image) {
        ...
        if (!Files.exists(fileStorageLocation)) {
            try {
                ...
                log.error(exception.getMessage());
                throw new ApiException("Could not create directory to save image");
            }
            log.info("Directory created successfully: {}" , fileStorageLocation);
        }
        try {
            Files.copy(image.getInputStream(), fileStorageLocation.resolve(email + ".png"), REPLACE_EXISTING);
        } catch (Exception exception) {
            log.error(exception.getMessage());
            throw new ApiException("Could not save image");
        }
